...
public class BlueTerm extends Activity {
	...
	public synchronized void locChange(double lo, double la) {
		...
		/*
		 * double longitude = 0.36 * (lo - 52.15517440); double latitude = 0.36
		 * * (la - 5.38720621); double SomX = (190094.945 * latitude) +
		 * (-11832.228 * longitude * latitude) + (-144.221 *
		 * Math.pow(longitude,2) * latitude) + (-32.391 * Math.pow(latitude,3))
		 * + (-0.705 * longitude) + (-2.340 * Math.pow(longitude,3) * latitude)
		 * + (-0.608 * longitude * Math.pow(latitude,30) + (-0.008 *
		 * Math.pow(latitude,2)) + (0.148 * Math.pow(longitude,2) *
		 * Math.pow(latitude,3))); double SomY = (309056.544 * longitude) +
		 * (3638.893 * Math.pow(latitude,2)) + (73.077 * Math.pow(longitude,2))
		 * + (-157.984 * longitude * Math.pow(latitude,2)) + (59.788 *
		 * Math.pow(longitude,3)) + (0.433 * latitude) + (-6.439 *
		 * Math.pow(longitude,2) * Math.pow(latitude,2)) + (-0.032 * longitude *
		 * latitude) + (0.092 * Math.pow(latitude,4)) + (-0.054 * longitude *
		 * Math.pow(latitude,4)); int RDX = (int)(155000 + SomX); int RDY =
		 * (int)(463000 + SomY);
		 */
		...
		/*
		 * try { InputStream inS =
		 * getBaseContext().getAssets().open("bomen.txt"); InputStreamReader inR
		 * = new InputStreamReader(inS); BufferedReader buffR = new
		 * BufferedReader(inR); String line; while ((line = buffR.readLine()) !=
		 * null) {
		 * 
		 * 
		 * 
		 * //line.substring(start, end); /*int xbegin =
		 * 1,xeind,ybegin,yeind,zbegin,zeind = line.length(); int i = 1; while
		 * (!String.valueOf(line.charAt(i)).equals(",")) { i++; } xeind = i - 1;
		 * ybegin = i + 1; while (!String.valueOf(line.charAt(i)).equals("]")) {
		 * i++; } yeind = i - 1; zbegin = i + 2; int x =
		 * Integer.parseInt(line.substring(xbegin,xeind + 1)),y =
		 * Integer.parseInt(line.substring(ybegin,yeind + 1)),z =
		 * Integer.parseInt(line.substring(zbegin,zeind)); //bijde negatief is
		 * links onder bijde positief is rechts boven double X = x - RDX; double
		 * Y = y - RDY; double d; //verdeel berekening in positieve en negatieve
		 * kwadaranten if (X < 0) { if (Y < 0) { d = Math.sqrt((x - RDX) * (x -
		 * RDX) + (y - RDY) * (y - RDY)); } else { d = Math.sqrt((x - RDX) * (x
		 * - RDX) + (y - RDY + 20) * (y - RDY + 20)); } } else { if (Y < 0) { d
		 * = Math.sqrt((x - RDX + 20) * (x - RDX + 20) + (y - RDY) * (y - RDY));
		 * } else { d = Math.sqrt((x - RDX + 20) * (x - RDX + 20) + (y - RDY +
		 * 20) * (y - RDY + 20)); } } if (d < 100) { bomen += z; } else if (d <
		 * 100 + Math.sqrt(800)) { d -= 100; d /= Math.sqrt(800); z =
		 * (int)Math.round(d * z); bomen += z; } } Log.e(LOG_TAG,"er zijn " +
		 * bomen + " bomen in de buurt"); //TODO bomen naar percentage Double
		 * percent = bomen.doubleValue() / 400.0 * 100.0; int perc =
		 * percent.intValue(); byte[] data = new byte[1]; data[0] = (byte)perc;
		 * if (mSerialService != null) {
		 * Log.e(LOG_TAG,"send data from locChange"); send(data); } } catch
		 * (IOException e) {}//
		 */
	}
	...
	/**
	 * Handle dpad left-right-up-down events. Don't handle dpad-center, that's
	 * our control key.
	 * 
	 * @param keyCode
	 * @param down
...
class TranscriptScreen implements Screen {
	 ...
	 * transcript that holds lines that have scrolled off the top of the screen.
	 */
...
interface TextRenderer {
	...
	void drawTextRun(Canvas canvas, float x, float y, int lineOffset, char[] text, int index, int count, boolean cursor, int foreColor, int backColor);
}
...
abstract class BaseTextRenderer implements TextRenderer {
	...
	protected int[] mBackPaint = {0xff000000, // Black -- is overridden by constructor
	0xffcc0000, // Red
	0xff00cc00, // green
	0xffcccc00, // yellow
	0xff0000cc, // blue
...
class EmulatorView extends View implements GestureDetector.OnGestureListener {
	...
	public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
		...
		return new BaseInputConnection(this,false) {
			...
			private final String KEYCODE_CHARS = "\000\000\000\000\000\000\000" + "0123456789*#" + "\000\000\000\000\000\000\000\000\000\000" + "abcdefghijklmnopqrstuvwxyz,." + "\000\000\000\000" + "\011 "   // tab, space
			...
			/**
			 * ChinnyxD edit: Vanuit hier kan data worden verzonden 0-255
			 * 
			 * @param c int data naar device
			 */
			private void sendChar(int c) {
				try {
					Log.d("BlueTerm","sendChar Test" + c);
					mapAndSend(c);
				}
	...
	public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
		// fagTODO: add animation man's (non animated) fling
	...
	public int keyDown(int keyCode, KeyEvent event) {
		...
		switch(keyCode) {
			...
			case KeyEvent.KEYCODE_DEL:
				...
				break;
			default: {
				result = event.getUnicodeChar((mCapKey.isActive() ? KeyEvent.META_SHIFT_ON:0) | (mAltKey.isActive() ? KeyEvent.META_ALT_ON:0));
				break;
			}
		}
